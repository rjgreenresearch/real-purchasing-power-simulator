...
# ---------------------------------------------------------------------------
# Catalog integrity: the splice's required series must be in FRED_SERIES
# ---------------------------------------------------------------------------

class TestCatalogIntegrity:
    """The wage and productivity splices each depend on specific FRED series.
    If those series are not in the bulk-download catalog, `make data` will
    appear to succeed (downloading 43-of-44 series) but then `nber_splice
    --build-spliced-dataset` will crash with FileNotFoundError. This test
    closes that gap by failing at test time instead.

    Regression for the v0.3.5 catalog-integrity fix.
    """

    def test_wage_legacy_series_in_catalog(self):
        from rpps.fred_loader import FRED_SERIES
        from rpps.nber_splice import WAGE_LEGACY_SERIES
        assert WAGE_LEGACY_SERIES in FRED_SERIES, (
            f"{WAGE_LEGACY_SERIES} is required by the wage splice "
            f"(rpps.nber_splice.build_wage_splice) but is not in "
            f"rpps.fred_loader.FRED_SERIES. `make data` will not download "
            f"it, and the splice will fail with FileNotFoundError."
        )

    def test_wage_modern_series_in_catalog(self):
        from rpps.fred_loader import FRED_SERIES
        from rpps.nber_splice import WAGE_MODERN_SERIES
        assert WAGE_MODERN_SERIES in FRED_SERIES

    def test_productivity_modern_series_in_catalog(self):
        from rpps.fred_loader import FRED_SERIES
        from rpps.nber_splice import PROD_MODERN_SERIES
        assert PROD_MODERN_SERIES in FRED_SERIES

    def test_compute_all_required_series_in_catalog(self):
        """Every series referenced by `rpps.metrics.compute_all` must also
        be in the catalog so `make data && make metrics` is a complete
        pipeline."""
        from rpps.fred_loader import FRED_SERIES

        # These are the series compute_all loads via fred_loader.load_series.
        required = ["CPIAUCNS", "OPHNFB", "COMPRNFB"]
        for sid in required:
